...
def vex_template_vgos_std(**kwargs):
    ...
    Message.addMessage(f"start adjusting vex file based on standard vgos template", dump="session")
    ...
    with open(backup_vex, "r") as vex_in, open(path_to_vex, "w") as vex_out:
        ...
        for l in vex_in:
            ...
            if read:
                ...
                vex_out.write(l)
        pass
    Message.addMessage(f"finished adjusting vex file based on standard vgos template", dump="session")
    pass
...
def fill_vex_template(**kwargs):
    ...
    # generate backup of vex file
    backup_vex = Path(path_selected) / (code + ".vex.orig.VieSchedpp")
    Message.addMessage(f"start filling VEX template", dump="session")
    Message.addMessage(f"    - generate backup of {path_to_vex} to {backup_vex}", dump="session")
    shutil.copy(path_to_vex, backup_vex)
    ...
    with open(path_to_vex, "w") as f:
        f.write(vex_new)
    Message.addMessage(f"finished filling VEX template", dump="session")
    pass


def _vex_in_sked_format(**kwargs):
    Message.addMessage("start converting .vex file to \"sked\" format for external parsers", dump="session")
    path_selected = kwargs["path"]
    code = kwargs["session"]["code"].lower()
    name_skd = (code + ".skd")
    name_vex = (code + ".vex")
    ...
    with open(newVex) as f:
        all = f.readlines()
        all[1] = "*  schedule generated by VieSched++, converted with sked\n"

    with open(newVex, 'w') as f:
        f.writelines(all)
    Message.addMessage(f"finished converting .vex file to \"sked\" format for external parsers", dump="session")

    pass


def _vlba_vex_adjustments(**kwargs):
    Message.addMessage("start adjusting .vex file for VLBA needs", dump="session")
    path_selected = kwargs["path"]
    code = kwargs["session"]["code"].lower()
    name_vex = (code + ".vex")
    ...
    try:
        ...
        Message.addMessage(f"    - change dir to {cwd}", dump="session")
        os.chdir(str(cwd))
    Message.addMessage("finished adjusting .vex file for VLBA needs", dump="session")
