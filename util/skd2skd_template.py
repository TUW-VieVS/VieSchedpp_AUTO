...
def skd2skd_template(path_source, path_template):
    ...
    software = re.compile(f'SCHEDULING_SOFTWARE\s+([\w+-]*)')
    version = re.compile(f'SOFTWARE_VERSION\s+([\w-]*)')
    created = re.compile(f'SCHEDULE_CREATE_DATE\s+(.*)\n')
    scheduler = re.compile(f'SCHEDULER\s+(\w*)')
    correlator = re.compile(f'CORRELATOR\s+(\w*)')
    start = re.compile(f'START\s+(\d*)')
    end = re.compile(f'END\s+(\d*)')

    template = re.sub(exper, exper.search(skd).group(), template, 1)
    template = re.sub(description, description.search(skd).group()[:-1] + "_skd_converted\n", template, 1)
    template = re.sub(software, software.search(skd).group(), template, 1)
    template = re.sub(version, version.search(skd).group(), template, 1)
    template = re.sub(created, created.search(skd).group(), template, 1)
    template = re.sub(scheduler, scheduler.search(skd).group(), template, 1)
    template = re.sub(correlator, correlator.search(skd).group(), template, 1)
    template = re.sub(start, start.search(skd).group(), template, 1)
    template = re.sub(end, end.search(skd).group(), template, 1)
